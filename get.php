...
$url = new moodle_url('/local/etl/get.php', array('plugin' => $plugin, 'key' => $key, 'mode' => $testmode, 'id' => $id));
...
if (!file_exists($CFG->dirroot.'/local/etl/plugins/'.$plugin.'/extractor.class.php')) {
    print_error('errornoplugin', 'local_etl');
}

require_once($CFG->dirroot.'/local/etl/plugins/'.$plugin.'/extractor.class.php');

$method = optional_param('method', 'des', PARAM_ALPHA);

try {
    $extractorclass = 'local_etl\\'.$plugin.'_extractor';
...
if (!empty($etl_environment)) {
    ...
    if ($testmode == 'test') {
        echo '</pre>';
        echo '<br/>';
        echo $OUTPUT->continue_button(new moodle_url('/local/etl/index.php', array('plugin' => $plugin)));
        echo $OUTPUT->footer();
    }
} else {
    if ($testmode == 'test') {
        print_error('errorkey', 'local_etl');
    } else {
