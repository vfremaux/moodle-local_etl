...
if (!empty($action)) {
    $result = include($CFG->dirroot.'/local/etl/managequeries.controller.php');
    if ($result == -1) {
        echo $OUTPUT->footer();
        return;
    }
...
echo $OUTPUT->heading(get_string('managequeries', 'local_etl'), 1);

$queries = $DB->get_records('local_etl_query', array('plugin' => $plugin));
...
if (!empty($queries)) {
    ...
    $querystr = get_string('query', 'local_etl');
    $cmdstr = get_string('cmds', 'local_etl');
    ...
    foreach ($queries as $query) {
        ...
        $queriesurl = new moodle_url('/local/etl/managequeries.php', $params);
    ...
    echo get_string('noqueries', 'local_etl');
}

$params = array( 'etlplugin' => $plugin, 'what' => 'add' );
echo $OUTPUT->single_button(new moodle_url('/local/etl/managequeries.php', $params), get_string('addaquery', 'local_etl'));

$opts = array( 'plugin' => $plugin);
echo $OUTPUT->single_button(new moodle_url('/local/etl/index.php', $opts), get_string('backtoetl', 'local_etl'));
