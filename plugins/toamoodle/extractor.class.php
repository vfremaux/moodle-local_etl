...
class toamoodle_extractor extends etl_extractor {
    ...
    function __construct($id, $key, $authmethod, $local = false) {
        ...
        if ($toa = $DB->get_record('localetl_'.self::$plugingname, array('id' => $id))) {
    ...
    function extract_test(&$output, $testmode = false) {
        ...
        $plugin = new StdClass;
        include_once($CFG->dirroot.'/local/etl/plugins/toamoodle/version.php');
        ...
        if ($rs) {
            foreach ($rs as $u) {
                ...
                if (preg_match('/^block_/', $u->module)) {
                    ...
                    // See later after course information retrieval.
                    $u->instance = get_string('nc', 'localetl_toamoodle', '');
                    $u->visible = 1;
                    $u->groupmode = null;
                } else {
                    // Should be a module.
                    if ($u->cmid) {
                        ...
                        $u->visible = get_string('nc', 'localetl_toamoodle');
                        $u->groupmode = null;
                        $u->instance = get_string('nc', 'localetl_toamoodle');
                    }
                }
                ...
                if (!array_key_exists($u->courseid, $COURSECACHE)) {
                    if (!$course = $DB->get_record('course', array('id' => $u->course))) {
                        $course->idnumber = '---';
                        $course->groupmode = get_string('nc', 'localetl_toamoodle');
                        $course->shortname = get_string('nc', 'localetl_toamoodle');
                ...
                switch ($u->groupmode) {
                    case NOGROUPS:
                        $u->groupmode = get_string('nogroups', 'localetl_toamoodle');
                        break;

                    case SEPARATEGROUPS:
                        $u->groupmode = get_string('separatedgroups', 'localetl_toamoodle');
                        break;

                    case VISIBLEGROUPS:
                        $u->groupmode = get_string('visiblegroups', 'localetl_toamoodle');
                        break;
                }
                ...
                if (function_exists('tao_is_learning_format') && tao_is_learning_format($course)){
                    ...
                    $u->classifier1 = get_string('nc', 'localetl_toamoodle');
                    $u->classifier2 = get_string('nc', 'localetl_toamoodle');
                    $u->classifier3 = get_string('nc', 'localetl_toamoodle');
                }
                ...
                unset($u->courseid);
                $visiblestr = get_string('visible', 'localetl_toamoodle');
                $nonvisiblestr = get_string('nonvisible', 'localetl_toamoodle');
    ...
    function extract_academics(&$output, $testmode = false) {
        ...
        // Context.
        $sitestr = get_string('sitecontext', 'localetl_toamoodle');
        $categorystr = get_string('categorycontext', 'localetl_toamoodle');
        $coursestr = get_string('coursecontext', 'localetl_toamoodle');
        $modulestr = get_string('modcontext', 'localetl_toamoodle');
        $blockstr = get_string('blockcontext', 'localetl_toamoodle');
        $userstr = get_string('usercontext', 'localetl_toamoodle');
        ...
        if ($rs) {
            foreach ($rs as $u) {
                ...
                $u->section = get_string('realassigns', 'localetl_toamoodle');
        ...
        if ($rs) {
            foreach ($rs as $u) {
                ...
                $u->from = $this->parms->from;
                $u->to = $this->parms->to;
                $u->context = get_string('nc', 'localetl_toamoodle');
                $u->contextlevel = -1;
                $u->instance = get_string('nc', 'localetl_toamoodle');
                $u->role = get_string('nc', 'localetl_toamoodle');
                $u->section = get_string('orphans', 'localetl_toamoodle');
                $u->object = get_string('nc', 'localetl_toamoodle');
        ...
        if ($rs) {
            foreach ($rs as $c) {
                ...
                $c->to = $this->parms->to;
                $c->username = get_string('nc', 'localetl_toamoodle');
                $c->section = get_string('unusedcontexts', 'localetl_toamoodle');
                $c->role = get_string('nc', 'localetl_toamoodle');
        ...
        if ($rs) {
            ...
            foreach ($rs as $u) {
                ...
                if ($u->courseid) {
                    ...
                    $u->course = get_string('nc', 'localetl_toamoodle');
                    $u->visible = true;
                }
