...
$url = new moodle_url('/local/etl/plugins/toamoodle/get.php');
...
if (!empty($etl_environment)) {
    ...
    if ($testmode == 'test') {
        echo '</pre>';
        echo '<br/>';
        print_continue(new moodle_url('/local/etl/index.php', array('plugin' => $plugin)));
        echo $OUTPUT->footer();
    }
} else {
    if ($testmode == 'test') {
        print_error('errorkey', 'localetl_toamoodle');
    } else {
