...
$url = new moodle_url('/local/etl/plugins/toamoodle/tests/sso_check.php');
...
$PAGE->set_url($url);
$PAGE->set_heading(get_string('sslcheck', 'localetl_toamoodle'));
$PAGE->set_pagelayout('admin');
$PAGE->navbar->add('TOA ETL Access check');

echo $OUTPUT->header();

echo $OUTPUT->heading(get_string('sslcheck', 'localetl_toamoodle'));

echo $OUTPUT->box_start();
echo get_string('sslcheckadvice', 'localetl_toamoodle');
echo $OUTPUT->box_end();
...
if (!$DB->record_exists('localetl_toamoodle', array('id' => $id))) {
    $toa = new StdClass();
    $toa->id = $id;
    ...
    $DB->set_field('localetl_toamoodle', 'publickey', $string_cert, array('id' => $id));
}
