...
$url = new moodle_url('/local/etl/plugins/toamoodle/tests/des_check.php');
...
$PAGE->set_heading(get_string('descheck', 'localetl_toamoodle'));
...
echo $OUTPUT->heading(get_string('descheck', 'localetl_toamoodle'));

echo $OUTPUT->box_start();
echo get_string('descheckadvice', 'localetl_toamoodle');
echo $OUTPUT->box_end();

$id = optional_param('toa', 1, PARAM_INT);

// sets up test realm 
if (! $toainstance = $DB->get_record('localetl_toamoodle', array('id' => $id))) {
    $toa = new StdClass;
    $toa->id = $id;
    $toa->wwwroot = $CFG->wwwroot;
    $toa->publickey = 'toamoodle';
    $DB->insert_record('localetl_toamoodle', $toa);
} else {
    if (empty($toainstance->publickey)) {
        $DB->set_field('localetl_toamoodle', 'publickey', 'toamoodle', array('id' => $id));
    }
}
...
echo '<input type="checkbox" name="mode" value="test" /> '.get_string('testmode', 'localetl_toamoodle').'<br/>';
echo '<input type="submit" name="go_btn" value="'.get_string('testextraction', 'localetl_toamoodle').'" />';
echo '</form>';
