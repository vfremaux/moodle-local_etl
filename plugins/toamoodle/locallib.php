...
function toa_get_upper_categories(&$u, &$course) {
    ...
    if (empty($course)) {
        ...
        for($i = 1; $i <= 4 ; $i++){
            $key = "category$i";
            $u->$key = get_string('nc', 'localetl_toamoodle');
        }
        return;
    }
    ...
    if (!array_key_exists($course->id, $COURSECATCACHE)){
        if ($course->id == 1 || $course->category == 0){
            ...
            if ($course->id == SITEID){
                $COURSECATCACHE[1][] = get_string('sitecat', 'localetl_toamoodle');
            } else {
                $COURSECATCACHE[1][] = get_string('coursecaterror', 'localetl_toamoodle');
            }
        } else {
            $categories = array();
...
function toa_get_user_roles(&$u, &$course, $userid = null, $extension = '', $context = null) {
    ...
    if (empty($userid)){
        $u->$allrolesfield = get_string('nc', 'localetl_toamoodle');
        $u->$hasdirectrolefield = get_string('nc', 'localetl_toamoodle');
        return;
    }
    ...
    switch ($u->contextlevel) {
        ...
        case CONTEXT_COURSECAT: // CATEGORY
            ...
            if ($cat = $DB->get_record('course_categories', array('id' => $u->instanceid))){
                ...
                $u->section = get_string('caterror', 'localetl_toamoodle');
            }
            break;
        case CONTEXT_COURSE: // COURSES
            if ($course = $DB->get_record('course', array('id' => $u->instanceid))){
                ...
                $u->section = get_string('courseerror', 'localetl_toamoodle');
            }
            break;
        case CONTEXT_MODULE: // MODULES
            if ($cm = $DB->get_record('course_modules', array('id' => $u->instanceid))){
                ...
                $u->section = get_string('moderror', 'localetl_toamoodle');
            }
            break;
        case CONTEXT_BLOCK: // BLOCKS
            if ($blockinstance = $DB->get_record('block_instance', array('id' => $u->instanceid))) {
                ...
                $u->section = get_string('blockerror', 'localetl_toamoodle');
            }
            break;
    }
...
function toa_get_user_info(&$u, $userid = null, $extension = '') {
    ...
    $ncstr = get_string('nc', 'localetl_toamoodle');
...
function toa_reshape_user_origin(&$u, $extension = '') {
    ...
    if ($u->$userhostidfield == 1) {
        $u->$userhostfield = get_string('local', 'localetl_toamoodle');
    } elseif ($u->$userhostidfield == 2) {
        $u->$userhostfield = get_string('allhosts', 'localetl_toamoodle');
    } else {
...
function toa_reshape_visible(&$u) {
    ...
    if (is_null($u->visible)) {
        ...
        $u->visible = ($u->visible) ? get_string('visible', 'localetl_toamoodle') : get_string('nonvisible', 'localetl_toamoodle');
    }
}
