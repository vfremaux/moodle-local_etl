...
$url = new moodle_url('/local/etl/plugins/boardz/tests/des_check.php');
...
$PAGE->set_heading(get_string('descheck', 'localetl_boardz'));
...
echo $OUTPUT->heading(get_string('descheck', 'localetl_boardz'));

echo $OUTPUT->box_start();
echo get_string('descheckadvice', 'localetl_boardz');
echo $OUTPUT->box_end();

$id = optional_param('boardz', 1, PARAM_INT);

// sets up test realm 
if (! $boardzinstance = $DB->get_record('localetl_boardz', array('id' => $id))) {
    $boardz = new StdClass;
    $boardz->id = $id;
    ...
    if (empty($boardzinstance->publickey)) {
        $DB->set_field('localetl_boardz', 'publickey', 'boardz', array('id' => $id));
    }
}

// get record back
$boardz = $DB->get_record('localetl_boardz', array('id' => $id));
