...
class boardz_extractor extends etl_extractor {
    ...
    public function __construct($id, $key, $authmethod, $local = false) {
        ...
        if ($boardz = $DB->get_record('localetl_'.self::$pluginname, ['id' => $id])) {
    ...
    public function extract_test(&$output, $testmode = false) {
        ...
        $plugin = new StdClass;
        include_once($CFG->dirroot.'/local/etl/plugins/boardz/version.php');
    ...
    public function extract_academics(&$output, $testmode = false) {
        ...
        // Context.
        $sitestr = get_string('sitecontext', 'localetl_boardz');
        $categorystr = get_string('categorycontext', 'localetl_boardz');
        $coursestr = get_string('coursecontext', 'localetl_boardz');
        $modulestr = get_string('modcontext', 'localetl_boardz');
        $blockstr = get_string('blockcontext', 'localetl_boardz');
        $userstr = get_string('usercontext', 'localetl_boardz');
        ...
        if ($rs) {
            foreach ($rs as $u) {
                ...
                $u->section = get_string('realassigns', 'localetl_boardz');
        ...
        if ($rs) {
            foreach ($rs as $u) {
                ...
                $u->from = $this->parms->from;
                $u->to = $this->parms->to;
                $u->context = get_string('nc', 'localetl_boardz');
                $u->contextlevel = -1;
                $u->instance = get_string('nc', 'localetl_boardz');
                $u->role = get_string('nc', 'localetl_boardz');
                $u->section = get_string('orphans', 'localetl_boardz');
                $u->object = get_string('nc', 'localetl_boardz');
        ...
        if ($rs) {
            foreach ($rs as $c) {
                ...
                $c->to = $this->parms->to;
                $c->username = get_string('nc', 'localetl_boardz');
                $c->section = get_string('unusedcontexts', 'localetl_boardz');
                $c->role = get_string('nc', 'localetl_boardz');
    ...
    */
    ...
    public function reset() {
        ...
        $oldrec = $DB->get_record('localetl_boardz', ['id' => $this->id]);
        ...
        $DB->update_record('localetl_boardz', $oldrec);
    }
}
