...
abstract class etl_extractor {
    ...
    public static function instance($plugin, $id, $key = null, $method = null, $local = false) {
        ...
        if (file_exists($CFG->dirroot.'/local/etl/plugins/'.$plugin.'/extractor.class.php')) {
            include_once($CFG->dirroot.'/local/etl/plugins/'.$plugin.'/extractor.class.php');
            $classname = "\\local_etl\\{$plugin}_extractor";
    ...
    public function config() {
        ...
        if (empty($this->name)) {
            print_error('cannotconfigure', 'local_etl');
            return;
        }

        $queries = $DB->get_records('local_etl_query', array('plugin' => $this->name));
    ...
    public function extract($testmode = false, &$results = null) {
        ...
        if (get_config('etlinprogress', 'etl') == 1) {
            if ($testmode == 'test') {
                $str .= get_string('etlbusy', 'localetl');
            } else {
        ...
        if (preg_match("/^special_(.*)/", $this->query, $matches)) {
            ...
            if (method_exists($this, $functionname)) {
                ...
                set_config('etlinprogress', 0, 'etl');
                print_error('errormissingfunction', 'local_etl', $functionname);
            }
        }
